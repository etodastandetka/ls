...
TRANSLATIONS = {
    'ru': {
        ...
        'select_casino': "Выберите:",
        'casino_label': "{casino_name}",
        'bank_label': "Банк: {bank_name}",
        'phone_label': "Телефон: {phone}",
        'enter_player_id': "Введите ваш ID игрока:",
        ...
        'enter_account_id': "Введите ваш ID игрока:",
        ...
        'error_processing_casino': "❌ Произошла ошибка при обработке выбора. Попробуйте еще раз или введите /start",
...
async def send_main_menu(bot, chat_id: int, user_name: str) -> None:
    ...
    # Отправляем сообщение с клавиатурой и скрываем текст после короткой паузы
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Не удалось скрыть текст сообщения меню: {delete_error}")
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if param:
        ...
        if param.startswith('ref'):
            ...
            # Проверяем, что код не пустой и содержит только цифры
            if not referral_code or not referral_code.strip().isdigit():
                logger.warning(f"⚠️ Неверный формат реферального кода '{param}': код пустой или содержит не только цифры")
            else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in user_states:
        ...
        if step == 'deposit_bookmaker':
            try:
                ...
                if not bookmaker_deposit_enabled:
                    ...
                    await update.message.reply_text(
                        f"❌ Пополнения для {casino_name} временно недоступны. Попробуйте позже или выберите другой счет.",
                        parse_mode='HTML'
                    )
        ...
        if step == 'withdraw_bookmaker':
            try:
                ...
                if not bookmaker_withdraw_enabled:
                    casino_name = get_casino_name(bookmaker)
                    await update.message.reply_text(
                        f"❌ Выводы для {casino_name} временно недоступны. Попробуйте позже или выберите другой счет.",
                        parse_mode='HTML'
                    )
        ...
        if step == 'deposit_player_id':
            ...
            # ВСЕГДА добавляем случайные копейки к сумме (1-99 копеек), игнорируя копейки пользователя
            # Это защита от скама - пользователь не может передать точную сумму с копейками
            base_amount = int(amount)  # Берем только целую часть, игнорируя копейки пользователя
            random_kopecks = random.randint(1, 99)
            amount = base_amount + (random_kopecks / 100)
            logger.info(f"💰 Сгенерированы случайные копейки: {random_kopecks}, итоговая сумма: {amount} (игнорированы копейки пользователя)")
                            ...
                            if omoney_url:
                                try:
                                    ...
                                    if QRCODE_AVAILABLE:
                                        ...
                                        # Текст "ПОПОЛНЕНИЕ" поверх QR-кода по диагонали
        ...
        # Обработка отправки фото чека для депозита (или выбора банка)
            ...
            try:
                ...
                if not data.get('bookmaker'):
                    ...
                    await update.message.reply_text("❌ Ошибка: отсутствует название счета. Начните заново.")
                    if user_id in user_states:
                        del user_states[user_id]
                    clear_pending_deposit_state(user_id)
                    return
