...
export async function GET(request: NextRequest) {
  try {
    ...
    if (topOnly) {
      try {
        ...
        const topReferrerIds = topReferrersRaw.length > 0 
          ? topReferrersRaw.map((r: { referrer_id: bigint }) => r.referrer_id)
          : []
      ...
      const userMap = new Map(topReferrerUsers.map((u: { userId: bigint; username: string | null; firstName: string | null }) => [u.userId.toString(), u]))
      
      const prizeDistribution = [10000, 5000, 2500, 1500, 1000]
      
      const topReferrers = topReferrersRaw.map((ref: { referrer_id: bigint; total_deposits: number | bigint; referral_count: bigint }, index: number) => {
        const user = userMap.get(ref.referrer_id.toString()) as { username: string | null; firstName: string | null } | undefined
    ...
    // Получаем данные пользователей для топ-5
    const topReferrerIds = topReferrersRaw.map((r: { referrer_id: bigint }) => r.referrer_id)
    ...
    const topReferrers = topReferrersRaw.map((ref: { referrer_id: bigint; total_deposits: number; referral_count: bigint }, index: number) => {
      const user = userMap.get(ref.referrer_id.toString()) as { username: string | null; firstName: string | null } | undefined
      const displayName = user?.username 
        ? `@${user.username}` 
        : user?.firstName 
    ...
    const topReferrersWithPrizes = topReferrers.map((ref: any, index: number) => ({
      ...ref,
      prize: prizeDistribution[index] || 0
    }))
    ...
    const totalWithdrawn = completedWithdrawals.reduce((sum: number, w: { amount: any }) => {
    ...
    const referralsList = await Promise.all(
      userReferrals.map(async (ref: any) => {
